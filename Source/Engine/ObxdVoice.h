...
public:
	...
	{
		briHold = val;
		brightCoef = tan(jmin(val,flt.SampleRate*0.5f-10) * (MathConstants<float>::pi) * flt.sampleRateInv);
	...
	void setSampleRate(float sr)
	{
		...
		SampleRate = sr;
		sampleRateInv = 1 / sr;
		brightCoef = tan(jmin(briHold,flt.SampleRate*0.5f-10) * (MathConstants<float>::pi) * flt.sampleRateInv);
	}
	void checkAdsrState()
	{
