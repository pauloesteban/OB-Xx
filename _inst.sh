...
echo ""
case $Inst in 
 1) mkdir -p $Opt/lib/lv2 && test ! -d $Opt/lib/lv2 && echo "ERROR; Couldn't create $Opt. Wrong permissions?" && echo "Try su/sudo to root and run $0 manually" && exit 2
 ...
 2) test -d $Loc/lib/lv2 || mkdir $Loc/lib/lv2 >& /dev/null
    test ! -w $Loc/lib/lv2 && echo "ERROR; Couldn't write to $Loc/lib/lv2 . Wrong permissions?" && echo "Try su/sudo to root and run $0 manually" && exit 3 
